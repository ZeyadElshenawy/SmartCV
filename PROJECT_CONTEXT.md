...
**System B — multi-job discovery** (`jobs/services/job_sources/`): dashboard "Scan now" → `ScrapeJob` → `runner.start_in_thread` spawns a **daemon thread** with its own asyncio loop + DB connection (`close_old_connections`; `DJANGO_ALLOW_ASYNC_UNSAFE=true` set here, the only place). **Seven sources** behind a uniform `(ScrapeQuery, ProgressReporter)` registry (`query.py` dataclass; legacy LinkedIn/Indeed/Glassdoor wrapped in `runner.py`): LinkedIn (Selenium if credentialed, else Playwright anon), Indeed (Playwright, Cloudflare-detect → `SourceBlocked`), Glassdoor (raises `SourceAuthRequired` without a saved session), **Wuzzuf** (plain requests+bs4, Egypt, detail-page descriptions), **Bayt** (Playwright, MENA/Gulf + international), **Remotive** & **Remote OK** (official JSON APIs, remote/global, run once per scan via `LOCATION_AGNOSTIC`). Per source×location → `JobListing` rows → `score_listings_for_user` → `RecommendedJob`. The runner writes a per-source outcome map to `ScrapeJob.source_status` (`ok/empty/blocked/auth_required/error` — migration 0008), surfaced as live chips in the dashboard scan panel + a "Last scan:" chip row; the final message names failing sources ("No listings — Glassdoor needs login; Indeed blocked"). Boards without a URL date facet honor `date_posted` by post-filtering card ages through `base.posted_within`.
...
**PDF (the source of truth).** `resumes/services/pdf_exporter.py` (105L): `generate_pdf(resume, output, template_name) :51` → `resolve_template(template_name) :33` → `templates/resumes/pdf_template_{theme}.html`, rendered by **WeasyPrint**. **Five** live themes (`LIVE_THEMES :15` = `ats_clean` B&W · `ats_clean_accent` · **`ats_dense`** one-page-tight · **`ats_spacious`** airy-slate · **`ats_strict`** no-rules) — each a `pdf_template_{name}.html` that extends `templates/resumes/pdf_base.html` and overrides only its `theme_css` block (ATS guarantees live in one place). Old theme names migrate via `THEME_MIGRATION` (never 500). Editor download is `export_pdf_view :921`; the picker (`template_choices`, set in the edit view) lists all five, saved as `content['template_name']`. **Stage 2 (the three new themes) is COMMITTED at HEAD** — each was a one-file addition because the shared-render preview (below) picks them up for free.
...
**DOCX.** `resumes/services/docx_exporter.py` (777L): `generate_docx(resume) :705` builds an ATS-clean DOCX with python-docx (mixed-case name title, 13 pt rule'd section headings, right-aligned dates, italic company line — a hierarchy match to the PDF). It is **single-style** — it ignores `template_name`. ⚠ **STILL DEFERRED:** the per-template DOCX "theme hook" — the five PDF/preview themes do **not** change the DOCX export (it always renders the one ATS-clean style). The Stage-2 *templates* shipped; only DOCX theme parity remains.
...
| **Job boards** (`job_sources/`) | Playwright (LinkedIn/Indeed/Glassdoor/Bayt) + Selenium LinkedIn + requests (Wuzzuf) + JSON APIs (Remotive/Remote OK) | saved `storage_state` JSON via `login_<source>` commands (LinkedIn/Indeed/Glassdoor only — the four new sources are anonymous); partial stealth | per-source/location try-except + typed `SourceBlocked`/`SourceAuthRequired`/`SourceError` exceptions → honest `ScrapeJob.source_status`; partial successes survive; `debug_dump()` HTML on selector miss; block markers only count when 0 cards parse (Bayt/Indeed embed benign Turnstile scripts) | **Mixed** — LinkedIn/Indeed/Glassdoor selectors rotate & Cloudflare blocks; Wuzzuf/Bayt server-rendered + fixture-tested (`jobs/test_job_sources.py`); Remotive/Remote OK are official APIs (most reliable) |
...
**`jobs/services/job_sources/` (multi-job discovery):** `__init__.py` (45), `auth.py` (33), `base.py` (252 — + `Source*` exceptions, `posted_within`, `html_to_text`), `query.py` (37 — `ScrapeQuery` dataclass), `runner.py` (406 — daemon-thread orchestrator, uniform registry, per-source status), `linkedin.py` (224), `linkedin_selenium.py` (461), `indeed.py` (273), `glassdoor.py` (354), `wuzzuf.py` (263), `bayt.py` (217), `remotive.py` (105), `remoteok.py` (135). Fixture-based tests: `jobs/test_job_sources.py` (349, 20 tests) + `jobs/testdata/` (live-captured 2026-06-11).
...
- **Scraper selectors** — LinkedIn/Kaggle/Indeed/Glassdoor break silently when the sites re-render (§11). Mitigated for discovery since 2026-06-11: per-source `ScrapeJob.source_status` labels blocked/auth/error sources in the UI, and Wuzzuf/Bayt/Remotive/Remote OK parsers are pinned by fixture tests (`jobs/test_job_sources.py`).
