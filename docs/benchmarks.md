...
| ATS matched-vs-mismatched separation | matched **100.0** vs mismatched **7.3333** (Cohen's d = **10.572**) | 3 matched, 6 mismatched | benchmarks/ats_eval.py |
| Endpoint warm p95 (max across routes) | **35.75 ms** | 5 routes x 100 req | benchmarks/latency_runner.py |
| CV parser personal-info accuracy | **0.847** | 25 CVs | benchmarks/parser_eval.py |
| CV parser skills F1 (CVs with explicit skills section) | **0.728** (Jaccard 0.566) | 23/25 CVs | benchmarks/parser_eval.py |
| CV parser skills F1 (all CVs, incl. those without a skills section) | 0.693 (Jaccard 0.534) | 25 CVs | benchmarks/parser_eval.py |
| Skill extractor F1 | **0.772** (P=0.807, R=0.747, halluc=0.193) | 30 JDs x 1 runs | benchmarks/skill_extractor_eval.py |
| Gap analyzer coverage (Phase 2 reconciliation) | **1.000** (150/150 pairs at 100%) | 150 (CV,JD) pairs | benchmarks/gap_eval.py |
| Gap analyzer separation (similarity score) | strong **0.2628** / partial **0.3214** / weak **0.2613** (Cohen's d strong-vs-weak = **0.078**) | 150 pairs | benchmarks/gap_eval.py |
| Tailored resume — judge axes (1-10) | factuality **4.3235** / relevance **4.5588** / ats_fit **4.5294** / human_voice **2.5** | 34 pairs (strong) | benchmarks/tailoring_eval.py |
| Tailored resume — programmatic entity grounding | **1.0** of generated entities appear verbatim in source CV | 34 pairs | benchmarks/tailoring_eval.py |
...
| --- | --- | --- |
| ats_eval | 0.01 | yes |
| latency_runner | 40.54 | yes |
| parser_eval | 8.38 | yes |
| skill_extractor_eval | 125.93 | yes |
| gap_eval | 2493.84 | yes |
| tailoring_eval | 2215.21 | yes |
