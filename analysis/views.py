...
def gap_analysis_view(request, job_id):
    ...
    if existing and not force_recompute:
        ...
        # Save to database
        gap_analysis, _ = GapAnalysis.objects.update_or_create(
            job=job,
            defaults={
...
def generate_learning_path_view(request):
    """Generate a personalized learning path based on missing skills across jobs"""

    # Single query: fetch all gap analyses for this user's jobs
